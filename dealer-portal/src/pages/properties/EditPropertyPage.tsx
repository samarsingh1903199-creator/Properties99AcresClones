...
import { ArrowLeft, Save, ImagePlus, X, Star, Loader2, Layers, ListChecks } from "lucide-react";
...
export function EditPropertyPage() {
    ...
    Promise.all([
      ...
      .then(([{ data: p }, aRes]) => {
        setForm({
          ...
          price: String(p.price), area: String(p.area),
          bedrooms: String(p.bedrooms), bathrooms: String(p.bathrooms),
          location: p.location, city: p.city, description: p.description,
          status: p.status as PropertyStatus,
        });
        setImages(p.images.map(url => ({ kind: "url" as const, url, name: url.split("/").pop() ?? "image" })));
        setCoverIndex(0);
        if (aRes?.data) setAmenitiesInit(amenitiesFromApi(aRes.data));
      })
  ...
  if (fetchError) return (
    <div className="text-center py-20 text-[#111111]/40">
      ...
      <Link to={ROUTES.PROPERTIES} className="text-[#5b21b6] font-bold hover:underline">← Back to properties</Link>
    </div>
  );
  ...
  return (
    <div className="max-w-2xl mx-auto pb-28">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-[#111111] tracking-tight" style={{ fontFamily: "Outfit, sans-serif" }}>Edit Property</h1>
          <p className="text-sm font-medium text-[#111111]/40 mt-0.5 truncate">{form.title}</p>
        </div>
      </div>

      {/* Tab bar */}
      <div className="flex gap-1 p-1 bg-gray-100/80 rounded-2xl mb-6">
        {([
          { key: "details",   label: "Property Details",   icon: Layers },
          { key: "amenities", label: "Amenities & Features", icon: ListChecks },
        ] as { key: Tab; label: string; icon: React.ElementType }[]).map(({ key, label: l, icon: Icon }) => (
          <button key={key} type="button" onClick={() => setTab(key)}
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[12px] font-black transition-all ${
              tab === key
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="dp-card p-6 space-y-4">
            ...
            <div>
              ...
              <input required value={form.title} onChange={e => set("title", e.target.value)} className="dp-input" style={{ borderRadius: "0.875rem" }} />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" required min="1" value={form.price} onChange={e => set("price", e.target.value)} className="dp-input" style={{ borderRadius: "0.875rem" }} />
              </div>
              <div>
                <label className={lbl}>Area (sq.ft)</label>
                <input type="number" required min="1" value={form.area} onChange={e => set("area", e.target.value)} className="dp-input" style={{ borderRadius: "0.875rem" }} />
              </div>
            </div>
            <div className="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select value={form.status} onChange={e => set("status", e.target.value)} className="dp-input dp-select" style={{ borderRadius: "0.875rem" }}>
                  {["active","pending","sold","rented","draft"].map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="dp-card p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input required value={form.location} onChange={e => set("location", e.target.value)} className="dp-input" style={{ borderRadius: "0.875rem" }} />
              </div>
              <div>
                <label className={lbl}>City</label>
                <input required value={form.city} onChange={e => set("city", e.target.value)} className="dp-input" style={{ borderRadius: "0.875rem" }} />
              </div>
            </div>
          </div>
          ...
          {submitError && (
            <p className="text-sm font-semibold text-red-600 bg-red-50 border border-red-100 rounded-xl px-4 py-3">{submitError}</p>
          )}
          ...
          <div className="fixed bottom-0 left-0 right-0 z-20 bg-white/90 backdrop-blur-md border-t border-[rgba(91,33,182,0.08)] px-4 py-3 flex justify-end gap-3 lg:left-64">
            ...
            <button type="submit" disabled={uploading}
              className="premium-btn flex items-center gap-2 px-5 h-11 text-xs tracking-widest disabled:opacity-60 disabled:cursor-not-allowed">
              {uploading ? <><Loader2 className="w-4 h-4 animate-spin" />SAVING…</> : <><Save className="w-4 h-4" />SAVE CHANGES</>}
            </button>
          </div>
        </form>
      )}
